...
- **Y-banded level parameters — gravity and palette** (`_Y_BANDS`, SWRAM build). Per-level sorted list of Y thresholds, each carrying a `gravity_FRAC` byte plus optional landscape and object colour overrides (`$FF` sentinel = inherit level default). `update_active_band` runs once per frame: scans the level's `band_y_HI` / `band_y_LO` arrays (terminated by `$FF` in the high byte), reseeds `gravity_FRAC` / `gravity_SIGN` from the level base, pre-fills colour targets with the level defaults, then overrides with the deepest crossed band's gravity / colour values. Band gravity bytes are sign-extended (bit 7 → `gravity_SIGN`) so `$80..$FF` give upward gravity for "hollow earth" mid-sections. Level-base gravity is also sign-extended at level init and on each frame's reseed, so negative base gravity is supported. Respawn passes the spawn-point Y through `update_active_band` before the orientation test, so dying inside a reverse-gravity band respawns the ship with the correct orientation. Palette is only re-applied when the deepest active band's *index* changes from one frame to the next (cached in `band_current_active_index`), so the existing `hide_landscape` / `show_landscape` paths can still drive the palette in between transitions without being fought every frame. After `level_reset` writes its level-default palette, the cache is invalidated and `update_active_band` is re-run so the post-respawn `player_teleport_appear` animation already renders in the band's colours. Editor: "Band" mode (`B`), draggable horizontal lines spanning full editor width, gravity ±1/±$10 with `[`/`]`/`,`/`.`, K/J cycle the selected band's landscape / object colour overrides (`inherit → 1..7 → inherit`), signed-decimal display on band labels and toolbar gravity widget. Exporter sorts bands ascending by Y and emits two parallel colour-override arrays alongside the gravity array. Other band overrides (X-wrap generalisation, ambient hazard flags, smooth blending) deferred. Non-SWRAM CRC remains anchored at `6389c446`.
