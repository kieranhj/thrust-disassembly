...
**Respawn logic needs revisiting.** `level_reset` currently picks the checkpoint with the smallest Y that's still above the death Y. That's wrong for disconnected caverns: if the only checkpoint inside a sub-cavern sits in its middle, dying in the top half of that cavern would match a higher checkpoint *above the divider band*, respawning the player back in the hub cavern with no way to return short of re-triggering the teleporter. Probably want a "last used checkpoint" model: track the most recent checkpoint the player actually arrived at (entered via teleporter or passed through during play) and respawn there by default. The existing height-based scan stays as a fallback for the very first life and as the rule for advancing through normal play (passing a new checkpoint should update the "last used" pointer). Needs a new ZP byte for the active checkpoint index, write sites in the teleporter warp path and wherever the existing checkpoint-advance logic lives, and a reset on level start.

**Pod-carry respawn flips the scan direction.** When the player is carrying the pod, the assumption is that they're heading *up* (back to the surface to escape), so `level_reset` picks the closest checkpoint *below* the death Y instead of above. This is an anti-cheat: without the flip, dying near the top of the level while carrying the pod would respawn at a high checkpoint, effectively granting free upward travel through the death-respawn loop. Whatever new "last used checkpoint" logic gets added needs to preserve this directional flip while the pod is held — probably by updating the "last used" pointer on pod pickup to the closest-below checkpoint, and using a different update rule (closest *below* the player) while the pod-held flag is set.
